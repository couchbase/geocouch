...
lookup_path_test() ->
    ...
    EntryPath2 = lookup_path(Fd, RootPos, {19,3,20,4}, 2),
    ?assertEqual(-1, EntryPath2),

    % Found in 2nd level
    EntryPath3 = lookup_path(Fd, RootPos, {458,700,687,865}, 2),
    ?assertEqual([1,0], EntryPath3),
    % Found in 3rd level (would also match [1,1,0], but we take the first
    % matching child)
    EntryPath4 = lookup_path(Fd, RootPos, {542,356,698,513}, 3),
    ?assertEqual([0,0,1], EntryPath4),
    EntryPath5 = lookup_path(Fd, RootPos, {542,656,598,813}, 3),
    ?assertEqual([0,1,0], EntryPath5),
    EntryPath6 = lookup_path(Fd, RootPos, {42,656,598,813}, 3),
    ?assertEqual([0,1,1], EntryPath6),
    EntryPath7 = lookup_path(Fd, RootPos, {342,456,959,513}, 3),
    ?assertEqual([1,1,0], EntryPath7).
